...
const StatCard = ({ label, value, color, icon, sub }) => (
  <div style={{
    ...
    onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.boxShadow = `0 8px 40px ${color}20`; }}
    onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = `0 0 30px ${color}10`; }}
  >
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    fetchAll();
    const interval = setInterval(fetchAll, 15000); // refresh tiap 15 detik
    return () => clearInterval(interval);
  }, []);

  const activityIcon = (type) => {
    const icons = {
      login: "🔑", register: "✨", workout_done: "💪",
      upgrade_premium: "⭐", logout: "🚪",
    };
    return icons[type] || "📌";
  };
  ...
  if (loading) return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh", color: "#e03030", fontSize: 18 }}>
      <div>
  ...
  return (
    <div style={{ padding: "40px", minHeight: "100vh", color: "white" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div>
            ...
            <p style={{ color: "#555", fontSize: 14, marginTop: 6 }}>
              Realtime overview — FIT-IN System
            </p>
          </div>
          <button onClick={fetchAll} style={{
            background: "rgba(224,48,48,0.1)", border: "1px solid rgba(224,48,48,0.3)",
            color: "#e03030", borderRadius: 12, padding: "10px 20px",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 20, marginBottom: 36 }}>
        ...
        />
        <StatCard label="Workout Selesai Hari Ini" value={actStats.today_workouts} color="#fb923c"
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          <h2 style={{ color: "#e03030", fontSize: 18, fontWeight: 700, margin: 0 }}>
            Aktivitas Terbaru
          </h2>
          <span style={{
            background: "rgba(224,48,48,0.1)", border: "1px solid rgba(224,48,48,0.2)",
            color: "#e03030", fontSize: 11, fontWeight: 700, padding: "4px 10px",
            borderRadius: 20, letterSpacing: 1,
          }}>● LIVE</span>
        </div>

        {actStats.recent_activities.length === 0 ? (
          <p style={{ color: "#444", textAlign: "center", padding: 20 }}>Belum ada aktivitas</p>
        ) : (
            ...
            {actStats.recent_activities.map((act, i) => (
              <div key={i} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <span style={{ color: "#555", fontSize: 13, marginLeft: 8 }}>{act.activity.replace("_", " ")}</span>
                </div>
                <span style={{
                  padding: "3px 10px", borderRadius: 20, fontSize: 11, fontWeight: 700,
                  background: `${roleColor(act.role)}20`, color: roleColor(act.role),
                }}>{act.role}</span>
